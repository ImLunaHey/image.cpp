// Derived from monatis/clip.cpp (MIT), modernized for GGML 0.9.x and image.cpp.
// See LICENSE and THIRD_PARTY.md in the image.cpp distribution.
...
struct clip_ctx * clip_model_load(const char * fname, const int verbosity = 1) {
    ...
    size_t ctx_size = 0;
    {
        ...
        for (int i = 0; i < n_tensors; ++i) {
            ...
            ctx_size += sizeof(struct ggml_tensor) + ggml_tensor_overhead();
    ...
    const size_t mem_req = get_mem_req_by_size(new_clip) + get_scr_buf_req_by_size(new_clip);
...
void clip_image_batch_preprocess(const clip_ctx * ctx, const int n_threads, const clip_image_u8_batch * img_inputs,
    ...
    for (size_t index = 0; index < img_inputs->size; ++index) {
        clip_image_preprocess(ctx, &img_inputs->data[index], &imgs_resized->data[index]);
    }
}
...
bool clip_text_encode(const clip_ctx * ctx, const int n_threads, const clip_tokens * tokens, float * vec,
    ...
    for (int il = 0; il < n_layer; il++) {
        ...
        // scratch allocation is handled by the compute arena;
    ...
    static_cast<int32_t *>(eot->data)[0] = static_cast<int32_t>(N - 1);
    ...
    if (ggml_graph_compute_with_ctx(ctx0, gf, n_threads) != GGML_STATUS_SUCCESS) {
        ggml_free(ctx0);
        return false;
    }
...
// print
    ...
    if (normalize) {
        double norm = 0.0;
        for (int i = 0; i < projection_dim; ++i) norm += static_cast<double>(vec[i]) * vec[i];
        norm = std::sqrt(norm);
        if (norm > 0.0) for (int i = 0; i < projection_dim; ++i) vec[i] /= static_cast<float>(norm);
    }
...
bool clip_image_batch_encode(const clip_ctx * ctx, const int n_threads, const clip_image_f32_batch * imgs, float * vec,
    ...
    for (int b = 0; b < batch_size; b++) {
        static_cast<int32_t *>(cls->data)[b] = b * num_positions;
    }
    ...
    for (int b = 0; b < batch_size; b++) {
        struct ggml_tensor * row = ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, 1);
        static_cast<int32_t *>(row->data)[0] = b;
        struct ggml_tensor * embedding = ggml_get_rows(ctx0, embeddings, row);
...
// print
    ...
    if (normalize) {
        for (int b = 0; b < batch_size; ++b) {
            float * current = vec + static_cast<size_t>(b) * projection_dim;
            double norm = 0.0;
            for (int i = 0; i < projection_dim; ++i) norm += static_cast<double>(current[i]) * current[i];
            norm = std::sqrt(norm);
            if (norm > 0.0) for (int i = 0; i < projection_dim; ++i) current[i] /= static_cast<float>(norm);
        }
    }
...
bool clip_zero_shot_label_image(struct clip_ctx * ctx, const int n_threads, const struct clip_image_u8 * input_img,
    ...
    for (int i = 0; i < n_labels; i++) {
        ...
        clip_text_encode(ctx, n_threads, &tokens, txt_vec.data(), false);
