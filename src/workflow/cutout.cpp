...
imagecpp_status copy_mask(const imagecpp_const_image_view &view, std::vector<uint8_t> &mask, imagecpp_error *error) {
    if (view.pixel_format != IMAGECPP_PIXEL_FORMAT_GRAY_U8) {
        return imagecpp::core::fail(error, IMAGECPP_STATUS_MODEL_ERROR,
                                    "segmentation workflow requires a GRAY_U8 mask");
    }
    size_t byte_count = 0;
    if (!checked_area(view.width, view.height, 1, byte_count)) {
        return imagecpp::core::fail(error, IMAGECPP_STATUS_OUT_OF_RANGE, "segmentation mask is too large");
    }
...
imagecpp_status resized_mask(const std::vector<uint8_t> &source, uint32_t source_width, uint32_t source_height,
    ...
    if (!checked_area(output_width, output_height, 1, output_size)) {
        return imagecpp::core::fail(error, IMAGECPP_STATUS_OUT_OF_RANGE, "workflow mask output is too large");
    }
...
imagecpp_status apply_mask(const imagecpp_const_image_view &source, const std::vector<uint8_t> &mask,
    ...
    if (source_pixel_size == 0) {
        return imagecpp::core::fail(error, IMAGECPP_STATUS_UNSUPPORTED,
                                    "cutout workflow supports GRAY_U8, RGB_U8, RGBA_U8, or BGRA_U8 images");
    }
    size_t output_size = 0;
    if (!checked_area(source.width, source.height, 4, output_size) ||
        mask.size() != static_cast<size_t>(source.width) * source.height) {
        return imagecpp::core::fail(error, IMAGECPP_STATUS_OUT_OF_RANGE, "cutout workflow output is too large");
    }
    output.resize(output_size);
...
imagecpp_status imagecpp_cutout(imagecpp_session *segment_session, const imagecpp_model *upscaler_model,
    ...
    *output = nullptr;
    ...
    try {
        ...
        if (selected.mask.width != image->width || selected.mask.height != image->height) {
            return imagecpp::core::fail(error, IMAGECPP_STATUS_MODEL_ERROR,
                                        "segmentation mask dimensions do not match the source image");
        }

        std::vector<uint8_t> full_mask;
        status = copy_mask(selected.mask, full_mask, error);
        if (status != IMAGECPP_STATUS_OK) {
            return status;
        }
        Crop crop{0, 0, image->width, image->height};
        if (options->crop_to_mask != 0 &&
            !mask_bounds(full_mask, image->width, image->height, options->padding, crop)) {
            return imagecpp::core::fail(error, IMAGECPP_STATUS_MODEL_ERROR,
                                        "selected segmentation mask has no foreground pixels");
        }
        ...
        if (options->upscale_factor > 1) {
            imagecpp_image_result *upscaled_raw = nullptr;
            status = imagecpp_upscale(upscaler_model, &working_image, options->upscale_factor, &upscaled_raw, error);
            if (status != IMAGECPP_STATUS_OK) {
                return status;
            }
        ...
        result->width = working_image.width;
        result->height = working_image.height;
        result->color_space = working_image.color_space;
        result->mask = std::move(working_mask);
        result->source_box = {static_cast<float>(crop.x), static_cast<float>(crop.y),
        ...
        status = apply_mask(working_image, result->mask, result->image, error);
        if (status != IMAGECPP_STATUS_OK) {
            return status;
        }
...
imagecpp_status imagecpp_cutout_result_info(const imagecpp_cutout_result *result, imagecpp_cutout_info *output,
    ...
    *output = {
        ...
        {sizeof(imagecpp_const_image_view), result->image.data(), result->image.size(), result->width, result->height,
         static_cast<size_t>(result->width) * 4, IMAGECPP_PIXEL_FORMAT_RGBA_U8, result->color_space},
        {sizeof(imagecpp_const_image_view), result->mask.data(), result->mask.size(), result->width, result->height,
         result->width, IMAGECPP_PIXEL_FORMAT_GRAY_U8, IMAGECPP_COLOR_SPACE_UNKNOWN},
        result->source_box,
